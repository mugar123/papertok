...
**Fin de la animación.** Un solo `onAnimationEnd` en la raíz, que ignora todo evento cuyo `target` no sea la raíz (las tarjetas y el héroe emiten los suyos, y suben). Página entrante: `data-page-motion` pasa a `rest`. Página saliente: `safeToRemove()`. Reloj de seguridad de `EXIT_SAFETY_MS = 700` ms desde que `present` es falso, que llama a `safeToRemove` si el `animationend` no llegó (pestaña en segundo plano, animación cancelada) — y que además oculta la raíz (`visibility: hidden`), porque `AnimatePresence` solo retira a un hijo cuando CADA registrante de su contexto de presencia se ha entregado, y un elemento `motion` interno cuya salida nunca se asienta dejaría esta raíz montada, fija y opaca, encima de la página viva. El reloj se ancla solo a `present` (no a `safeToRemove`, cuya identidad cambia en cada render de `AnimatePresence`, lo que reiniciaría el reloj); la función se guarda en una ref para leer siempre la última. El reloj se limpia si `present` vuelve a verdadero (navegar de vuelta a la misma clave mientras salía): entonces la página vuelve a calcular su movimiento, se hace visible de nuevo, y sigue.

**Chunk frío.** Cada `PageTransition` envuelve a sus hijos en su propio `<Suspense fallback={<RouteFallback />}>`. Un chunk que no está en caché suspende solo a la página que llega, dentro de su raíz. La página retenida NO espera a eso: su `hold` dura 220 ms igual y luego se entrega sola, haya resuelto o no la que llega su chunk; así que un chunk frío puede dejar hasta ~100 ms en blanco entre que la retenida se va y el fallback se revela (`RouteFallback` conserva su retraso de 320 ms), no cero. Sigue siendo mejor que `mode="wait"`: el fallback llega ~200 ms antes de lo que llegaba ahí. El `Suspense` exterior de `App.jsx` se queda como red. El Explorer está precargado en reposo (`EntityExplorer.preload()`), así que el caso frío es el toque en los primeros segundos o una conexión que pide ahorro de datos.
...
Frente a los criterios de §9: barra en todos los fotogramas, cumplido en los siete (0 fotogramas sin barra, frente a «todos menos el primero» de hoy); cero fotogramas vacíos, cumplido en los siete (frente a 2 de hoy); entidad quieta antes de 260 ms, cumplido solo en las tres vueltas (226, 229, 221 ms) y no en las tres entradas ni en la pestaña (293–305 ms, 33–45 ms por encima del objetivo: el término dominante son los ~45–75 ms que pasan entre el toque y el primer fotograma de la animación —el commit de la ruta y el montaje de la página nueva en el hilo principal—, que las vueltas también pagan (180 ms de `pageLeave` → 221–229 ms); el fotograma de una sola página en `rest` llega además uno o dos `requestAnimationFrame` después de que la retenida se retira al acabar su `hold` de 220 ms. Acortar duraciones no alcanzaría los 260 ms; el coste de montaje sí); feed retenido sin moverse durante la ida, cumplido (opacidad 1 y `transform: none` constantes en las cuatro hojas de ida, sin un solo fotograma en que la que se queda cambie); tarjetas en reposo al volver, cumplido (`motion: rest`, opacidad 1 y `transform: none` constantes en las tres vueltas); pestaña sin hueco, cumplido (16 fotogramas con las dos páginas y 0 vacíos, frente a los ≈184 ms medidos antes).
