...
// of the app. The app now lives at /feed; the scope root is the landing,
// which has no offline shell to warm here. Fetching /feed during install
// closes that gap: by the time this worker activates, `papertok-html`
// already holds a shell for the app.
//
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches
      .open('papertok-html')
      .then((cache) => cache.add(new URL('feed', self.registration.scope).href))
      .catch(() => {}),
  )
})
