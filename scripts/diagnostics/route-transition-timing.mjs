...
try {
  ...
  while (Date.now() - s < 30000) { ok = await cdp.eval(`(()=>{const b=[...document.querySelectorAll('button')].find(x=>/Not now|Ahora no/i.test(x.textContent)); b&&b.click(); return !!document.querySelector(${JSON.stringify(SELECTOR)});})()`).catch(() => false); if (ok) break; await sleep(300); }
