...
test('the hero settles between its heights instead of snapping at the handover', async () => {
  ...
  assert.match(jsx, /const heroBodyRef = useRef\(null\);[\s\S]*?useHeightSettle\(\s*heroBodyRef,\s*\[isLoadingEntity, entity, orcidInfo, isLoadingOrcid, recentImpact, hasLoadedWikiImage\],\s*\{ enabled: !prefersReducedMotion, suspended: settleSuspended, easing: 'cubic-bezier\(0\.4, 0, 0\.2, 1\)' \},\s*\);/);
