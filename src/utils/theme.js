 ...
 * white flash on every load for anyone reading in the dark. One further page
 * that never imports this module carries its own independent copy for the same
 * reason: `privacy.html` (the privacy policy). Both inline copies also repaint
 * the `theme-color` meta. Three copies in all — this module plus those two
 * inline scripts, which `grep -rl papertok_theme` lists. Keep them in step:
 * this module is the one that must be right, but the inline copies are the
 * ones seen first.
 */
