...
test('the file is named after the paper, safely', () => {
  assert.equal(
    exportFileName({ title: 'Correladores & el 100% del ruido_medido' }),
    'correladores-el-100-del-ruido-medido-plain-words.tex',
  );
  assert.equal(exportFileName({ title: 'Ñandú en Ávila' }), 'nandu-en-avila-plain-words.tex');
  // A title made entirely of punctuation must still produce a filename.
  assert.equal(exportFileName({ title: '///' }), 'paper-plain-words.tex');
  assert.equal(exportFileName({}), 'paper-plain-words.tex');
});

test('the file name can carry another extension for the other formats', () => {
  assert.equal(
    exportFileName({ title: 'Ñandú en Ávila' }, 'pdf'),
    'nandu-en-avila-plain-words.pdf',
  );
  assert.equal(exportFileName({}, 'pdf'), 'paper-plain-words.pdf');
});
...
test('documentCopy hands back the English strings', () => {
  assert.match(documentCopy().provenance, /Rewritten by PaperTok/);
  assert.equal(documentCopy().levels.university, 'university');
});

test('la fecha se compone en inglés', () => {
  const day = new Date(Date.UTC(2026, 8, 4));
  assert.equal(formatExportDate(day), '4 September 2026');
});

test('una fecha que no lo es no rompe el documento', () => {
  // El export no puede caerse por esto: si la fecha falta, la línea la pierde.
  assert.equal(formatExportDate(new Date('nada')), '');
...
test('los metadatos salen montados', () => {
  ...
  assert.equal(meta.level, 'University level');
  assert.equal(meta.noticeLabel, 'Notice');
  assert.equal(meta.source, 'Original article: https://arxiv.org/abs/2405.04331 · 4 September 2026');
  ...
  assert.deepEqual(meta.colophon.rows.map(row => row.key), [
    'Original article', 'Authors', 'Source', 'This version', 'Annotated with',
  ]);
  assert.equal(meta.colophon.rows.at(-1).value, '2 highlights and 4 notes: 3 yours, 1 from the AI');
});

test('la mitad que vale cero no se imprime', () => {
  // Un documento con notas y ningún subrayado suelto es el caso normal, y
  // «0 highlights and 4 notes» es exactamente lo que no se puede imprimir.
  const copy = documentCopy();
  assert.equal(copy.annotatedWith({ marks: 0, mine: 2, ai: 2 }), '4 notes: 2 yours, 2 from the AI');
  assert.equal(copy.annotatedWith({ marks: 3, mine: 0, ai: 0 }), '3 highlights');
  assert.equal(copy.annotatedWith({ marks: 1, mine: 1, ai: 0 }), '1 highlight and 1 note: 1 yours, 0 from the AI');
  assert.equal(copy.annotatedWith({ marks: 0, mine: 0, ai: 0 }), 'No highlights or notes');
});

test('sin enlace al original la línea de fuente no queda coja', () => {
  const meta = documentMeta({
    paper: { title: 'T', authors: [] },
    level: 'beginner', originalUrl: '',
    generatedAt: new Date(Date.UTC(2026, 8, 4)),
    counts: { marks: 0, mine: 0, ai: 0 },
  });
  assert.equal(meta.source, '4 September 2026');
  assert.equal(meta.byline, '');
  assert.equal(meta.colophon.rows.find(row => row.key === 'Source'), undefined);
  assert.equal(meta.colophon.rows.at(-1).value, 'No highlights or notes');
});

test('el nivel y el recuento cambian con los datos, no solo el título', () => {
  const meta = documentMeta({
    paper: { title: 'T', authors: [{ name: 'A. B.' }] },
    level: 'researcher', originalUrl: 'https://doi.org/10.1/x',
    generatedAt: new Date(Date.UTC(2026, 8, 4)),
...
test('el título del colofón y el de la cabecera se recortan cada uno a su propio tope; el de portada no', () => {
  ...
  const meta = documentMeta({
    ...
    level: 'university', originalUrl: '',
    generatedAt: new Date(Date.UTC(2026, 8, 4)),
  });
  assert.equal(meta.title, long);
  assert.equal(meta.runningTitle, `${'Y'.repeat(45)}…`);
  const titleRow = meta.colophon.rows.find(row => row.key === 'Original article');
